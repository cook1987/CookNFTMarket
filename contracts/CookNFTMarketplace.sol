 ...
 */
contract CookNFTMarketplace is ReentrancyGuard,Initializable, OwnableUpgradeable {
    using Math for uint256;

    /**
     * 当前合约所在链 对应的price feed 合约代称
     */
    string public constant BASE_TOKEN_SYMBOL="BASE";
    ...
    function buyNFT(uint256 listingId, address erc20Token, uint256 amount) external payable nonReentrant returns(bool) {
        ...
        if(erc20Token != address(0) && amount > 0){
            ...
            uint price = uint(priceConsumer.getLatestPrice(token.symbol()));
            // 检查数据是否存在且有效
            ...
            // 使用 ETH
            uint price = uint(priceConsumer.getLatestPrice(BASE_TOKEN_SYMBOL));
            // 检查数据是否存在且有效
            require(price > 0, "Chainlink: price <= 0");
            uint needAmount = listing.price.ceilDiv(price);
    ...
    function endAuction(uint256 auctionId) external nonReentrant {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            if(auction.token != address(0)){
                ...
                uint256 price = uint256(priceConsumer.getLatestPrice(token.symbol()));
